...
10, 8, 6, 4 or 2 g/L (A blocks: A = pMeOx, A* = pEtOx). A cubist regression model for predictions of continuous LE values in % and a treebag classification model listing the probability of LE ≥ 80% are listed.""")
...
with st.spinner('Computing loading efficiencies, please wait...'):
    ...
    try:
        ...
        with col1:
            ...
            st.dataframe(df1.style.hide_index().background_gradient(axis=None, vmin=50, vmax=100, cmap="Reds"),height=215)
            st.caption("""Treebag probabilities for LE ≥ 80% (BAcc = 91 %)""")
            st.dataframe(df2.style.hide_index().background_gradient(axis=None, vmin=50, vmax=100, cmap="Reds"),height=215)
        with col2:
            st.image(im)
