...
with st.form(key='my_form_to_submit'):
    ...
    with col1:
        SMI2 = st.text_input('Potential co-formulated drug:', 'None')
    with col2:
        number = st.number_input('Drug feed of co-formulated drug',min_value=0, max_value=12, value="min", step=2)
...
if submit_button:   
    ...
    with st.spinner('CALCULATING PREDICTIONS (STEP 4 OF 4)...'):
        ...
        SDc = int(df2["DF"])*(int(df2["LE"])/100)
        SDc2 = ((int(df2["LC"])/100)*10)/(1-(int(df2["LC"])/100))
        SDcx = (SDc+SDc2)/2
