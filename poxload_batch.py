...
def main():
    ...
    uo=pd.read_csv(sys.argv[1])
    
    NAMES.extend(uo["name"])
    SMILES.extend(uo["smiles"])
    ...
    try:
        ...
        command=str("Rscript "+CON+"/create_formulations2.R > /dev/null 2>&1")
        os.system(command)
        ...
        command=str("Rscript "+CON+"/predict4.R > /dev/null 2>&1")
        os.system(command)
